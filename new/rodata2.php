...
$br = new BinaryReader(fopen('/Users/Marijn/Projects/Firered/baserom.gba', 'rb'));


$br->setPosition(0x472133);

while ($br->getPosition() < 0x47553C) {
    $pos = $br->getPosition();
    $code = $br->readUInt8();
    ...
    if ($code == 0x10) {
        ...
        try {
            ...
            printf("0x%X: 0x%X bytes (0x%X bytes compressed)<br>", $pos, strlen($data), $br->getPosition() - $pos);

        } catch (Exception $e) {
        }
        $br->setPosition($pos + 1);
        echo str_pad('',4096)."\n";
        flush();
    }
}
    ...
    */
    ...
    try {
        ...
        if (($size % 0x20) === 0) {
            ...
            $len2 = floor(strlen($data) / 0x20) * 0x20;
            if ($len2 > 1) {
                $data = substr($data, 0, $len2);
                printf('<td>%s</td>', dumpCompressedPalette($file, $offset, $data));
                printf('<td>%s</td>', dumpCompressedImage($offset, $data, '4bpp', $reader4, 4));
                printf('<td>%s</td>', dumpCompressedImage($offset, $data, '8bpp', $reader8, 4));
                printf('<td>%s</td>', dumpCompressedMap($offset, $data));
